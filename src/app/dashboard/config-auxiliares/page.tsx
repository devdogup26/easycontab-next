...
export default function ConfigAuxiliaresPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ ...form, tipo: activeTab, aliquota: form.aliquota ? parseFloat(form.aliquota) : null })
      })
      if (res.ok) {
        fetchItems()
        closeModal()
  ...
  async function handleDelete(id: string) {
    ...
    try {
      const res = await fetch(`/api/config-auxiliares/${id}?type=${activeTab}`, { method: 'DELETE' })
  ...
  function openModal(item?: ConfigItem) {
    if (item) {
      setEditingItem(item)
      setForm({ codigo: item.codigo, descricao: item.descricao, tipo: item.tipo || '', aliquota: item.aliquota?.toString() || '' })
    } else {
      setEditingItem(null)
  ...
  const filteredItems = items.filter(item =>
    item.codigo.toLowerCase().includes(search.toLowerCase()) || item.descricao.toLowerCase().includes(search.toLowerCase())
  )
  ...
  const getColumnTitle = () => {
    switch (activeTab) {
      case 'cfop': return 'Código CFOP'
      case 'ncm': return 'Código NCM'
      case 'cst': return 'Código CST'
      case 'aliquotas': return 'Código'
    ...
    <div className={styles.page}>
      ...
      <div className={styles.toolbar}>
        <div className={styles.searchBar}>
          <Search size={18} />
          <input type="text" placeholder={`Buscar ${activeTab.toUpperCase()}...`} value={search} onChange={(e) => setSearch(e.target.value)} className={styles.searchInput} />
        </div>
        ...
        <table className={styles.table}>
          ...
          <tbody>
            {filteredItems.map(item => (
              <tr key={item.id}>
                <td><code className={styles.code}>{item.codigo}</code></td>
                <td>{item.descricao}</td>
                {activeTab === 'aliquotas' && <td>{item.aliquota ? `${item.aliquota}%` : '-'}</td>}
                <td>
                  <div className={styles.actions}>
                    <button className={styles.editBtn} onClick={() => openModal(item)}><Edit size={16} /></button>
                    <button className={styles.deleteBtn} onClick={() => handleDelete(item.id)}><Trash2 size={16} /></button>
                  </div>
                </td>
              </tr>
            ))}
            {filteredItems.length === 0 && (
              <tr><td colSpan={activeTab === 'aliquotas' ? 4 : 3} className={styles.emptyRow}>Nenhum registro encontrado</td></tr>
            )}
          </tbody>
        </table>
      </div>
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modal}>
            <div className={styles.modalHeader}>
              <h2>{editingItem ? 'Editar' : 'Novo'} {activeTab.toUpperCase()}</h2>
            ...
            <form onSubmit={handleSubmit} className={styles.form}>
              ...
              {activeTab === 'aliquotas' && (
                <div className={styles.formGroup}>
                  ...
                  <input type="number" step="0.01" value={form.aliquota} onChange={(e) => setForm({ ...form, aliquota: e.target.value })} />
                </div>
              )}
              <div className={styles.formActions}>
                <button type="button" onClick={closeModal} className={styles.cancelBtn}>Cancelar</button>
                <button type="submit" className={styles.saveBtn}><Check size={16} /><span>{editingItem ? 'Salvar' : 'Criar'}</span></button>
              </div>
            </form>
          </div>
        </div>
