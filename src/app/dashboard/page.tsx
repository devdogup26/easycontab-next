...
export default async function DashboardPage() {
  ...
  const parcelamentoAlert = {
    ...
    pgfnAtraso: parcelamentos.filter(p => p.tipo === 'PGFN' && p.parcelasEmAtraso > 0)
      .reduce((sum, p) => sum + p.parcelasEmAtraso, 0),
    simplesTotal: parcelamentos.filter(p => p.tipo === 'SIMPLES_NACIONAL').length,
    simplesAtraso: parcelamentos.filter(p => p.tipo === 'SIMPLES_NACIONAL' && p.parcelasEmAtraso > 0)
      .reduce((sum, p) => sum + p.parcelasEmAtraso, 0),
    simplificadoTotal: parcelamentos.filter(p => p.tipo === 'SIMPLIFICADO').length,
    simplificadoAtraso: parcelamentos.filter(p => p.tipo === 'SIMPLIFICADO' && p.parcelasEmAtraso > 0)
      .reduce((sum, p) => sum + p.parcelasEmAtraso, 0)
  }
