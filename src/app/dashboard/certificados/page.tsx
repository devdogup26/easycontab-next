...
export default async function CertificadosPage() {
  ...
  const stats = {
    ...
    validos: certificados.filter(c => c.status === 'VALIDO' && new Date(c.validade) > em30Dias)
      .length,
    proximoVencimento: certificados.filter(
      c => c.status === 'VALIDO' && new Date(c.validade) <= em30Dias && new Date(c.validade) > hoje
    ).length,
    vencidos: certificados.filter(
      c => c.status === 'VENCIDO' || (c.status === 'VALIDO' && new Date(c.validade) <= hoje)
    ).length,
  };
    ...
    <div className={sharedStyles.page}>
      <div className={sharedStyles.header}>
        <div className={sharedStyles.headerContent}>
          <h1 className={sharedStyles.title}>Certificados Digitais</h1>
      ...
      <CertificadosClient certificados={certificadosData} stats={stats} tudoOk={tudoOk} />
    </div>
  );
}
