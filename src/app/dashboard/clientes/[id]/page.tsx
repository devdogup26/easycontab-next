        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <span className="text-xs text-slate-400 uppercase tracking-wide block">Razão Social</span>
                <p className="text-slate-800 mt-1">{cliente.nomeRazao}</p>
              </div>

              {cliente.nomeFantasia && (
                <div>
                  <span className="text-xs text-slate-400 uppercase tracking-wide block">Nome Fantasia</span>
                  <p className="text-slate-800 mt-1">{cliente.nomeFantasia}</p>
                </div>
              )}

              <div>
                <span className="text-xs text-slate-400 uppercase tracking-wide block">Regime Tributário</span>
              ...
              <div>
                <span className="text-xs text-slate-400 uppercase tracking-wide block">{cliente.tipoPessoa === 'PF' ? 'CPF' : 'CNPJ'}</span>
                <p className="text-slate-800 mt-1 font-mono">{formatDocumento(cliente.documento)}</p>
              </div>
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {cliente.logradouro && (
                  <div className="md:col-span-2">
                    <span className="text-xs text-slate-400 uppercase tracking-wide block">Logradouro</span>
                ...
                {cliente.cidade && (
                  <div>
                    <span className="text-xs text-slate-400 uppercase tracking-wide block">Cidade</span>
                    <p className="text-slate-800 mt-1">{cliente.cidade}{cliente.uf ? ` - ${cliente.uf}` : ''}</p>
                  </div>
                )}

                {cliente.cep && (
                  <div>
                    <span className="text-xs text-slate-400 uppercase tracking-wide block">CEP</span>
          ...
          {cliente.email || cliente.telefone || cliente.responsavelTecnico ? (
            <div className="card p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {cliente.email && (
                  <div>
                    <span className="text-xs text-slate-400 uppercase tracking-wide block">E-mail</span>
                ...
                {cliente.telefone && (
                  <div>
                    <span className="text-xs text-slate-400 uppercase tracking-wide block">Telefone</span>
                ...
                {cliente.responsavelTecnico && (
                  <div className="md:col-span-2">
                    <span className="text-xs text-slate-400 uppercase tracking-wide block">Responsável Técnico</span>
          ...
          <div className="card p-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between py-2 border-b border-slate-200">
                ...
                <span className="text-slate-800 font-medium">{cliente._count?.obrigacoes || 0}</span>
              </div>
              <div className="flex items-center justify-between py-2 border-b border-slate-200">
                <span className="text-slate-400">Parcelamentos</span>
                <span className="text-slate-800 font-medium">{cliente._count?.parcelamentos || 0}</span>
              </div>
              <div className="flex items-center justify-between py-2 border-b border-slate-200">
