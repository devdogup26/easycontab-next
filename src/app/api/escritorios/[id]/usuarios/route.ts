...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!nome || !email || !senha || !tipoPerfil || !login) {
    return NextResponse.json({ error: 'Nome, email, senha, tipo de perfil e login são obrigatórios' }, { status: 400 })
  }
  ...
  if (!perfilObj) {
    return NextResponse.json({ error: `Perfil ${tipoPerfil} não encontrado para este escritório` }, { status: 400 })
  }
  ...
  if (existingLogin) {
    return NextResponse.json({ error: `Login "${login}" já existe neste escritório` }, { status: 400 })
  }
  ...
  try {
    ...
    if (error.code === 'P2002') return NextResponse.json({ error: 'Login ou email já cadastrado' }, { status: 409 })
    throw error
  }
}
